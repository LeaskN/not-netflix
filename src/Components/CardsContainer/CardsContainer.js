...
export default function CardsContainer({ media }) {
  ...
  const maturityRatingUrl = media.media_type === 'movie' ?
    `https://api.themoviedb.org/3/movie/${media.id}/release_dates?api_key=fed5b7c4a0fea83e14866a8dd8cb6baa` :
    `https://api.themoviedb.org/3/tv/1408/content_ratings?&api_key=fed5b7c4a0fea83e14866a8dd8cb6baa`
        ...
        if (req.results) {
          for (let i = 0; i < req?.results?.length; i++) {
            if (req.results[i].iso_3166_1 === 'US') {
              return media.media_type === 'movie' ? req.results[i].release_dates[0].certification :  req.results[i].rating;
            }
          }
